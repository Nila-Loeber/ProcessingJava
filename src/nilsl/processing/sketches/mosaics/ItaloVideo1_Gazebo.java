...
public class ItaloVideo1_Gazebo extends MosaicEditorApplet {
	...
	public void setup() {
		...
		// mosDrawer = new TintDrawer(mosInfo, new TintInfo(255, 255, 255, 200),
		// 60);
		...
		// mosDrawer.imageEnhancers.add(new
		// ProcessingFilterEnhancer(PShader.POSTERIZE,5));
		// mosDrawer.imageEnhancers.add(new
		// ProcessingFilterEnhancer(PShader.THRESHOLD,0.7f));

		// mosDrawer.imageEnhancers.add(new
		// CutupEnhancer(settings.mosInfo.ydim));

		// VideoClipInfo clipInfo1=new VideoClipInfo(new
		// Interval(37*1000,38*1000),20);
		VideoClipInfo clipInfo2 = new VideoClipInfo(new Interval(37 * 1000,
				53 * 1000), settings.mosInfo.Size());
		// VideoClipInfo clipInfo2=new VideoClipInfo(new
		// Interval(40*1000,60*1000),settings.mosInfo.Size()/2);
		List<VideoClipInfo> clipInfos = new ArrayList<VideoClipInfo>();
		// clipInfos.add(clipInfo1);
		clipInfos.add(clipInfo2);
		imageProvider = new VideoImageProvider(
				"/Users/Nils/Documents/Processing/Video_Mosaic/Gazebo - I Like Chopin (WWF Club 9).mp4",
				clipInfos);
