...
public class ItaloVideo3_GazeboAlternated extends ViewerApplet {
	...
	public void setup() {
		...
		List<VideoClipInfo> clipInfos1 = Arrays.asList(new VideoClipInfo(
				new Interval(600, 2 * 1000), 5), new VideoClipInfo(
				new Interval(26500, 38 * 1000), 5), new VideoClipInfo(
				new Interval(60 * 1000, 62 * 1000), 5));

		List<VideoClipInfo> clipInfos2 = Arrays.asList(new VideoClipInfo(
				new Interval(37000, 42000), 15)
		...
		VideoImageProvider vip2 = new VideoImageProvider(
				"/Users/Nils/Documents/Processing/Video_Mosaic/Gazebo - I Like Chopin (WWF Club 9).mp4",
				clipInfos2);
		vip2.setFilterProcessor(new FilterProcessor(
				new ArrayList<FilterCommand>()));
		vip2.applyFilters();

		AlternatingImageProvider imageProvider = new AlternatingImageProvider(
				Arrays.asList(vip1, vip2));
