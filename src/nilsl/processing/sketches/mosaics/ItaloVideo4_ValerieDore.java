...
public class ItaloVideo4_ValerieDore extends MosaicEditorApplet {
	...
	public void setup() {
		...
		List<VideoClipInfo> clipInfos1 = Arrays.asList(new VideoClipInfo(
				new Interval(800, 2 * 1000), 6), new VideoClipInfo(
				new Interval(26500, 38 * 1000), 5), new VideoClipInfo(
				new Interval(60 * 1000, 62 * 1000), 5), new VideoClipInfo(
				new Interval(68700, 70 * 1000), 5), new VideoClipInfo(
				new Interval(94500, 96 * 1000), 5), new VideoClipInfo(
				new Interval(1000, 230 * 1000), 8));

		VideoImageProvider vip1 = new VideoImageProvider(videoBasePath
				+ "Valerie_Dore_-_The_Night_original_version_HD_HQ.mp4",
				clipInfos1);
		vip1.setFilterProcessor(new FilterProcessor(
				new ArrayList<FilterCommand>()));
		vip1.applyFilters();

		// VideoImageProvider vip2 = new
		// VideoImageProvider("/Users/Nils/Documents/Processing/Video_Mosaic/Gazebo - I Like Chopin (WWF Club 9).mp4",clipInfos2);
		// vip2.setFilterProcessor(new FilterProcessor(new
		// ArrayList<FilterCommand>()));
		// vip2.ApplyFilters();
		//
		// imageProvider = new
		// AlternatingImageProvider(Arrays.asList(vip1,vip2));
