...
public class ItaloRecord3_Tint extends MosaicEditorApplet {
	...
	public void setup() {
		...
		// mosDrawer = new
		// TintDrawer(mosInfo,tintInfos,settings.mosInfo.xdim*settings.mosInfo.ydim,1);
		...
		// mosDrawer.imageEnhancers.add(new
		// ProcessingFilterEnhancer(PShader.INVERT,0));
		// mosDrawer.imageEnhancers.add(new
		// ProcessingFilterEnhancer(PShader.BLUR ,5));
		// mosDrawer.imageEnhancers.add(new
		// TypoEnhancer("c:\\data\\fonts\\Aharoni-Bold-120.vlw",60,100,100,color(255,0,255,127),new
		// CounterProvider(settings.mosInfo.xdim*settings.mosInfo.ydim)));
		...
		try {
			imageProvider = new FilterableMultiImageProvider(
					"c:\\data\\unsorted\\recordimages.dat");
			imageProvider
					.setFilterProcessor(new FilterProcessor(filterCommands));
