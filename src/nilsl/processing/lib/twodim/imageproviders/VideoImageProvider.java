...
public class VideoImageProvider extends MultiImageProvider implements
	...
	static final Logger logger = LogManager.getLogger(VideoImageProvider.class
			.getName());
	...
	private void GenerateImages(List<VideoClipInfo> clips) {
		clips.forEach(clipInfo -> images.addAll(GetImages(clipInfo))); // TODO:
																		// test
																		// .parallelStream
																		// or
																		// .parallel
	...
	private List<NImage> GetImages(VideoClipInfo clip) {
		...
		for (int i = 0; i < clip.numPictures; i++) {
			logger.trace(String.format(
					"Generating Frame %d of %d. Jumping to: %f", i + 1,
					clip.numPictures, (clipStart + i * period)));
			video.jump(clipStart + i * period);
			if (video.available())
				video.read();
			PImage pImage = PAppletShim.getApplet().createImage(video.width,
					video.height, PApplet.RGB); // TODO
