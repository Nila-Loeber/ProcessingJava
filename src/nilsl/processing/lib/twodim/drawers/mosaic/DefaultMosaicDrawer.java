...
public class DefaultMosaicDrawer extends EnhanceableMosaicDrawer2d {
	...
	static final Logger logger = LogManager.getLogger(DefaultMosaicDrawer.class
			.getPackage().getName());
	...
	public DefaultMosaicDrawer(MosaicInfo mosInfo) {
		...
		this.parentApplet = mosInfo.applet;
	}
	...
	public void draw() {
		...
		do {
			...
			canvas.image(pg, counter.getCurX() * imgXSize, counter.getCurY()
					* imgYSize);
			logger.trace(String.format("Drawing tile %d:%d called.",
					counter.getCurX(), counter.getCurY()));
			counter.inc();
