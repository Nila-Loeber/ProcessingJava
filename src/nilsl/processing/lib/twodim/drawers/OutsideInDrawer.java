...
public class OutsideInDrawer extends Drawer {
	...
	static final Logger logger = LogManager.getLogger(OutsideInDrawer.class.getPackage().getName());
	Counter1d counter;
	...
	public void draw() {
		...
		counter.reset();		
		int increment = (Math.min(drawerInfo.imgSizeX, drawerInfo.imgSizeY)-Math.min(drawerInfo.imgSizeXdest, drawerInfo.imgSizeYdest))/drawerInfo.numPics;
		do{
			...
			int height=drawerInfo.imgSizeY-counter.getCurPos()*(increment);
			...
			logger.trace(String.format("Drawing image to %d:%d. Width, height: %d, %d. Increment: %d. Enhancers found: ",drawXPos,drawYPos,width,height,increment,imageEnhancers.size()));
			counter.inc();
		}
