...
public class TintDrawer extends EnhanceableMosaicDrawer2d implements Zoomable {
	...
	public TintDrawer(MosaicInfo mosInfo, List<TintInfo> tintInfos, int numPics, float repetitionFactor) {
	...
	public void draw() {
		...
		for (int i=0; i<numPics; i++){
			TintInfo currentTintInfo = tintInfos.get((int) (i/repetitionFactor % tintInfos.size()));
			
			parentApplet.tint(currentTintInfo.v1,currentTintInfo.v2,currentTintInfo.v3,currentTintInfo.opacity);
